...
public sealed class WorkItemEndpointTests
{
    ...
    /// <summary>
    /// Guards the fix for consolidation path: when K8s Job creation fails after ClaimAsync
    /// succeeds for a consolidation item, SafeRequeueAsync is called on a Dispatched item.
    /// Without Dispatched→Pending support the item would be stuck until EnforceDispatchedTimeoutAsync
    /// marks it Failed, losing the retry entirely.
    /// </summary>
    [Fact]
    public async Task RequeueWorkItem_DispatchedState_TransitionsToPending()
    {
        var entity = SeedEntity(WorkItemStatus.Dispatched);
        var response = await _client.PostAsync($"/api/work-items/{entity.Id}/requeue", null);

        // Dispatched → Pending is valid for consolidation recovery
        response.StatusCode.Should().Be(HttpStatusCode.OK);
