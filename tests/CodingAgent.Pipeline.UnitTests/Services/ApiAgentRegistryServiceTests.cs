...
public sealed class ApiAgentRegistryServiceTests
{
    ...
    private static AgentEntryDto MakeDto(string id = "agent-1", AgentStatus status = AgentStatus.Idle) =>
        AgentEntryDtoFactory.From(MakeEntry(id, status), run: null);
    ...
    public async Task GetAllAgents_AfterRefresh_ReturnsFetchedList()
    {
        ...
        _client.Setup(c => c.GetAgentsAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<AgentEntryDto> { AgentEntryDtoFactory.From(agent, null) } as IReadOnlyList<AgentEntryDto>);
