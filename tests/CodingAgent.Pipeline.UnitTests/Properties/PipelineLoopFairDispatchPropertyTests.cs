...
public class PipelineLoopFairDispatchPropertyTests
{
    ...
    public async Task ProviderFailureIsolation_SubsequentTemplatesStillDispatched(IsolationInput input)
    {
        ...
        var config = new PipelineConfiguration
        {
            ...
            // Disable the floor allocation for this test: the floor reserves MinIssueSlots slots
            // from the priority-loop budget, which would reduce priorityBudget below templateCount
            // when Budget == templateCount (the minimum generated by IsolationArbitraries).
            // This test is about provider failure isolation, not floor allocation, so the floor
            // must not interfere with the assumption that budget >= templateCount guarantees
            // each template at least one dispatch opportunity.
            MinIssueSlots = 0,
        };
