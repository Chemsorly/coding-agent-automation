...
public sealed class ApiOrchestrationDiTests : IAsyncLifetime
{
    ...
    // TODO [WARNING]: There is no test for the Redis-present path of ResolveRedisStoreOrNull.
    // A regression where the method returns null even when IConnectionMultiplexer is registered
    // would not be caught by this file. Add a companion test that registers a mock
    // IConnectionMultiplexer and asserts the return value is a non-null IRedisStore.
    // See review finding [WARNING] ApiOrchestrationDiTests.cs:210 (TestQualityReviewer review).
    ...
    private ServiceProvider BuildProvider()
    {
        ...
        services.AddSingleton<IWorkItemTransitionStore>(sp => new PostgresWorkItemTransitionStore(
            sp.GetRequiredService<IDbContextFactory<PipelineDbContext>>(),
            sp.GetRequiredService<WorkItemTransitionService>()));
        // TODO [WARNING]: PostgresWorkItemTransitionStore is constructed directly inside this AddSingleton
        // factory lambda rather than registered under its concrete type. GetRequiredService<PostgresWorkItemTransitionStore>()
        // would throw, and the registration cannot participate in keyed or decorated service resolution.
        // This is consistent with the broader pattern in this file, but if a downstream consumer ever
        // needs to resolve the concrete type, it will fail at runtime rather than at build time.
        // See review finding [WARNING] ApiOrchestrationDiTests.cs:307 (Correctness + DotNetSpecialist review).
