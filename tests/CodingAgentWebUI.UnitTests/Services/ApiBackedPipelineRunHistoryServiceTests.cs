...
public sealed class ApiBackedPipelineRunHistoryServiceTests
{
    ...
    public async Task GetRunHistoryAsync_Unpaged_ReturnsItems()
    {
        ...
        _client
            .Setup(c => c.GetRunHistoryAsync(1, 1000, false, false, It.IsAny<PipelineStep?>(), It.IsAny<CancellationToken>()))
    ...
    public async Task GetRunHistoryAsync_Paged_DelegatesPageAndSize()
    {
        ...
        _client
            .Setup(c => c.GetRunHistoryAsync(2, 10, false, false, It.IsAny<PipelineStep?>(), It.IsAny<CancellationToken>()))
    ...
    public async Task GetRunHistoryAsync_FeedbackOnly_PassesFlagToClient()
    {
        ...
        _client
            .Setup(c => c.GetRunHistoryAsync(1, 20, true, false, It.IsAny<PipelineStep?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(expected);

        var sut = CreateSut();
        var result = await sut.GetRunHistoryAsync(page: 1, pageSize: 20, feedbackOnly: true);

        _client.Verify(c => c.GetRunHistoryAsync(1, 20, true, false, It.IsAny<PipelineStep?>(), It.IsAny<CancellationToken>()), Times.Once);
