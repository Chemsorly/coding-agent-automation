...
public sealed class AgentHubBehaviorTests
{
    ...
    [Fact]
    public async Task ReportJobCompleted_FinalLabelNeedsRefinement_OverridesFailedStep()
    {
        var agent = CreateAgent();
        var run = CreateRun();
        var payload = new JobCompletionPayload { FinalStep = PipelineStep.Failed, FinalLabel = AgentLabels.NeedsRefinement, CompletedAt = DateTimeOffset.UtcNow };
    ...
    public async Task ReportJobCompleted_FinalLabelWontDo_OverridesCompletedStep()
    {
        var agent = CreateAgent();
        var run = CreateRun();
        var payload = new JobCompletionPayload { FinalStep = PipelineStep.Completed, FinalLabel = AgentLabels.WontDo, CompletedAt = DateTimeOffset.UtcNow };
    ...
    [Fact]
    public async Task ReportJobCompleted_NullFinalLabel_FallsBackToFinalStep()
    {
        var agent = CreateAgent();
        var run = CreateRun();
        var payload = new JobCompletionPayload { FinalStep = PipelineStep.Failed, FinalLabel = null, CompletedAt = DateTimeOffset.UtcNow };
