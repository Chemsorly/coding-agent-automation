...
public sealed class AgentJobLifecycleServiceCompletionTests
{
    ...
    public async Task Consolidation_transitions_agent_to_Idle()
    {
        ...
        _facade.Setup(f => f.GetRun("job-1")).Returns(run);
        // ClearAgentState now consolidates the TransitionStatus(Idle) + ActiveJobId clear
        // into a single call. Simulate the side effect so the ActiveJobId assertion passes.
        _facade.Setup(f => f.ClearAgentState("agent-1"))
            .Callback(() => agent.ActiveJobId = null);
        var svc = CreateService();

        await svc.HandleJobCompletedAsync(new JobId("job-1"), agent, payload, CancellationToken.None);

        _facade.Verify(f => f.ClearAgentState("agent-1"), Times.Once);
