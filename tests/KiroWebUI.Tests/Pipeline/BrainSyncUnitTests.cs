...
public class BrainSyncUnitTests
{
    ...
    public void BrainUpdateService_Validate_NoChanges_ReturnsWarnings()
    {
        ...
        var result = service.Validate("/fake", "run-1", Array.Empty<string>());
    ...
    [Fact]
    public void BrainUpdateService_BuildCommitMessage_ContainsRunIdAndIssue()
    {
        var message = BrainUpdateService.BuildCommitMessage("run-abc", "#42");

        message.Should().Contain("run-abc");
        message.Should().Contain("#42");
    }

    [Fact]
    public void BrainUpdateService_ResolveConflictAcceptBoth_PreservesBothSides()
    {
        var resolved = BrainUpdateService.ResolveConflictAcceptBoth(
            "ours content", "theirs content");

        resolved.Should().Contain("ours content");
        resolved.Should().Contain("theirs content");
        resolved.Should().Contain("merge: accepted both sides");
    }

    [Fact]
    public void BrainUpdateService_EnsureGitignoreEntry_AddsWhenMissing()
    {
        var result = BrainUpdateService.EnsureGitignoreEntry("node_modules/\n", ".brain/");
    ...
    public void BrainUpdateService_EnsureGitignoreEntry_SkipsWhenPresent()
    {
        ...
        var result = BrainUpdateService.EnsureGitignoreEntry(original, ".brain/");
    ...
    {
        var result = BrainUpdateService.EnsureGitignoreEntry("", ".brain/");

        result.Should().Contain(".brain/");
    }

    [Fact]
    public void BrainUpdateService_BuildFallbackLogEntry_ContainsRunIdAndFiles()
    {
        var files = new List<string> { "general/lessons-learned.md", "technology/dotnet.md" };
        var entry = BrainUpdateService.BuildFallbackLogEntry("run-xyz", "2026-07-19", files);

        entry.Should().Contain("run-xyz");
        entry.Should().Contain("general/lessons-learned.md");
        entry.Should().Contain("technology/dotnet.md");
        entry.Should().Contain("auto-fallback");
    }
