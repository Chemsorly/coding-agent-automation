...
public class PipelineOrchestrationServiceTests
{
    ...
    [Fact]
    public async Task StartPipeline_WithRiskTierSkip_SkipsReviewLoop()
    {
        ...
        _mockConfigStore.Setup(s => s.LoadPipelineConfigAsync(It.IsAny<CancellationToken>()))
            ...
            {
                WorkspaceBaseDirectory = Path.GetTempPath(),
                CodeReview = new CodeReviewConfiguration { Enabled = true, MaxIterations = 3, RiskTiers = new CodeReviewRiskTiers { Skip = new RiskThreshold { MaxFiles = 5, MaxLines = 30 } } }
            });
    ...
    [Fact]
    public async Task StartPipeline_WithRiskTierStandard_RunsReview()
    {
        var files = Enumerable.Range(0, 10).Select(i => new FileChangeSummary("Modified", $"src/File{i}.cs", 20, 10)).ToArray();
        _mockRepoProvider.Setup(p => p.GetFileChangesAsync(It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync(files);

        _mockConfigStore.Setup(s => s.LoadPipelineConfigAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PipelineConfiguration
            {
                WorkspaceBaseDirectory = Path.GetTempPath(),
                CodeReview = new CodeReviewConfiguration { Enabled = true, MaxIterations = 1, RiskTiers = new CodeReviewRiskTiers { Skip = new RiskThreshold { MaxFiles = 5, MaxLines = 30 } } }
            });
    ...
    [Fact]
    public async Task StartPipeline_WithSecurityPathMatch_ReturnsFull()
    {
        _mockRepoProvider.Setup(p => p.GetFileChangesAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
        ...
        _mockConfigStore.Setup(s => s.LoadPipelineConfigAsync(It.IsAny<CancellationToken>()))
            ...
            {
                WorkspaceBaseDirectory = Path.GetTempPath(),
                CodeReview = new CodeReviewConfiguration { Enabled = true, MaxIterations = 1, RiskTiers = new CodeReviewRiskTiers { Skip = new RiskThreshold { MaxFiles = 100, MaxLines = 1000 }, SecurityPaths = ["auth/", "crypto/"] } }
            });
    ...
    [Fact]
    public async Task StartPipeline_WithNullRiskTiers_ReturnsStandard()
    {
        _mockConfigStore.Setup(s => s.LoadPipelineConfigAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(new PipelineConfiguration
            {
                WorkspaceBaseDirectory = Path.GetTempPath(),
                CodeReview = new CodeReviewConfiguration { Enabled = true, MaxIterations = 1, RiskTiers = null }
            });
    ...
    [Fact]
    public async Task StartPipeline_WithRiskTierSkip_LogsSkipReason()
    {
        _mockRepoProvider.Setup(p => p.GetFileChangesAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
