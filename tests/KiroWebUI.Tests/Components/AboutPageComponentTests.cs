...
public class AboutPageComponentTests : BunitContext
{
    ...
    private PipelineOrchestrationService CreateService(string? runsDir = null)
    {
        ...
        return new PipelineOrchestrationService(
            ...
            runsDirectory: runsDir ?? Path.Combine(Path.GetTempPath(), $"about-test-{Guid.NewGuid()}"));
    }

    private static string CreateRunsDirectory(params PipelineRunSummary[] summaries)
    {
        var dir = Path.Combine(Path.GetTempPath(), $"about-test-{Guid.NewGuid()}");
        Directory.CreateDirectory(dir);
        foreach (var s in summaries)
            File.WriteAllText(Path.Combine(dir, $"{s.RunId}.json"), JsonSerializer.Serialize(s, JsonOptions));
        return dir;
    }
