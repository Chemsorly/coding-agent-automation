...
namespace CodingAgent.Web.IntegrationTests.DbMode;
...
public sealed class DbModeLifecycleEndToEndTests : IDisposable
{
    ...
    {
        // Arrange: Create WorkItem directly as Dispatched (as WorkItemDispatchLoop does after
    ...
    /// and the <c>Pending → Dispatched</c> transition that <c>WorkItemDispatchLoop</c>
    ...
    public async Task PendingQueueRestore_Regression_WorkItemEnqueuesAsPending_ThenTransitionsToDispatched()
    {
        ...
        // ── Phase 2: WorkItemDispatchLoop picks up Pending → transitions to Dispatched ────
        // Simulate WorkItemDispatchLoop.ExecuteDispatchLifecycleAsync:
        ...
        transitioned.Should().BeTrue("TransitionIfAsync(Pending → Dispatched) must succeed — " +
            "this is the CAS claim step inside WorkItemDispatchLoop.ExecuteDispatchLifecycleAsync");

        await using (var db = await _dbFactory.CreateDbContextAsync())
        {
            var item = await db.WorkItems.FindAsync(runId);
            item!.Status.Should().Be(WorkItemStatus.Dispatched, "WorkItem must be Dispatched after WorkItemDispatchLoop claims it");
    ...
    /// dispatched before a lower-weight one of the same task type when WorkItemDispatchLoop
    ...
    public async Task PendingQueueRestore_Regression_PriorityWeight_HigherWeightClaimedFirst()
    {
        ...
        // Act: simulate WorkItemDispatchLoop claiming items.
        ...
        await using (var db = await _dbFactory.CreateDbContextAsync())
        {
            ...
            high!.Status.Should().Be(WorkItemStatus.Dispatched,
                "higher-priority item must be claimed first by WorkItemDispatchLoop");
