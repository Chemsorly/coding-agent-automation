...
public class K8sLifecycleIntegrationTests : IDisposable
{
    ...
    public async Task ReconcileStartupLabelsAsync_InfrastructureFailure_DoesNotSwapToAgentNext()
    {
        ...
        // TODO: [BUG-10 review] Missing boundary test: add a test where InfrastructureFailure item has
        // CompletedAt outside the StartupReconciliationWindowMinutes (e.g., 10 min ago with 5-min window)
        // to verify it does NOT get re-labeled. Currently only the "within window" path is tested.
        var workItemId = Guid.NewGuid();
