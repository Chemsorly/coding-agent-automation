...
public class PipelineOrchestrationServiceTests : IDisposable
{
    ...
    public async Task StartPipeline_WarnAndExclude_PopulatesBlacklistedFilesAndCompletes()
    {
        ...
        _mockRepoProvider.Setup(p => p.CommitAllAsync(It.IsAny<WorkspacePath>(), It.IsAny<string>(), It.IsAny<IReadOnlyList<string>?>(), It.IsAny<CancellationToken>(), It.IsAny<IReadOnlyList<string>?>()))
    ...
    public async Task StartPipeline_ValidatesAllProvidersBeforeClone()
    {
        ...
        _mockRepoProvider.Setup(p => p.CloneAsync(It.IsAny<WorkspacePath>(), It.IsAny<CancellationToken>())).Callback(() => callOrder.Add("RepoProvider.CloneAsync")).Returns(Task.CompletedTask);
    ...
    public async Task StartPipeline_WhenProviderValidationFails_FailsWithClearErrorNamingProvider(string providerKind)
    {
        ...
        _mockRepoProvider.Verify(p => p.CloneAsync(It.IsAny<WorkspacePath>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task StartPipeline_DisposesPreviousProvidersBeforeCreatingNewOnes()
    {
        ...
        firstIssueProvider.Setup(p => p.GetIssueAsync(It.IsAny<IssueIdentifier>(), It.IsAny<CancellationToken>())).ReturnsAsync(new IssueDetail { Identifier = "42", Title = "Test", Description = "Desc", Labels = Array.Empty<string>() });
        firstIssueProvider.Setup(p => p.ListCommentsAsync(It.IsAny<IssueIdentifier>(), It.IsAny<CancellationToken>())).ReturnsAsync(new List<IssueComment>());
    ...
    {
        _mockRepoProvider.Setup(p => p.CloneAsync(It.IsAny<WorkspacePath>(), It.IsAny<CancellationToken>()))
    ...
    private static BrainTestContext CreateBrainTestService()
    {
        ...
        mockBrainProvider.Setup(p => p.CloneAsync(It.IsAny<WorkspacePath>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        mockBrainProvider.Setup(p => p.PullAsync(It.IsAny<WorkspacePath>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
    ...
    public async Task ConfidenceGate_NotReadyPostsAnalysisCommentFirst()
    {
        ...
        // TODO: Setup matcher uses string literal "42" relying on implicit string→IssueIdentifier conversion.
        // If the implicit conversion is removed in Phase 2, these tests will break at compile time. Update to use IssueIdentifier explicitly.
        ...
        _mockIssueProvider.Setup(p => p.PostCommentAsync("42", It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .Callback<IssueIdentifier, string, CancellationToken>((_, body, _) => commentOrder.Add(body.Contains("Agent Analysis") ? "analysis" : "gate"))
            .ReturnsAsync((string?)null);
    ...
    public async Task StartPipeline_WhenNoCommitsAhead_FailsWithNoChangesMessage()
    {
        ...
        _mockRepoProvider.Setup(p => p.HasCommitsAheadAsync(It.IsAny<WorkspacePath>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(false);
    ...
    public async Task StartPipeline_WithBrainProvider_IncludesBrainSyncSteps()
    {
        ...
        ctx.MockBrainProvider.Verify(p => p.CloneAsync(It.IsAny<WorkspacePath>(), It.IsAny<CancellationToken>()), Times.Once);
    ...
    public async Task StartPipeline_WhenBrainPreRunSyncFails_ContinuesWithoutBrain()
    {
        ...
        var ctx = CreateBrainTestService();
        ctx.MockBrainProvider.Setup(p => p.CloneAsync(It.IsAny<WorkspacePath>(), It.IsAny<CancellationToken>()))
    ...
    public async Task StartPipeline_WhenCodeGenThrowsGenericException_ContinuesToQualityGates()
    {
        ...
        _mockRepoProvider.Setup(p => p.GetFileChangesAsync(It.IsAny<WorkspacePath>(), It.IsAny<CancellationToken>()))
    ...
    public async Task StartPipeline_NoChangesAfterCleanup_WithExternalCi_SkipsCiAndCompletes()
    {
        ...
        _mockRepoProvider.Setup(p => p.CommitAllAsync(
            ...
            .Returns<WorkspacePath, string, IReadOnlyList<string>?, CancellationToken, IReadOnlyList<string>?>((_, _, _, _, _) =>
            {
                commitCallCount++;
        ...
        _mockRepoProvider.Setup(p => p.GetHeadCommitShaAsync(It.IsAny<WorkspacePath>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync("abc123");
    ...
    [Fact]
    public async Task ReleaseActiveAgentRunsAsync_NeverInvokesCancellationSender_Legacy()
    {
        // Retained from the original CancelActiveAgentRunsAsync_SendsCancelJobToAgents_BeforeLabelSwap test.
        // Verifies the repurposed behavior: sender is wired but release path never calls it.
        // The dedicated DoesNotSendCancelJobToAgents_WhenSenderIsWired test below covers the same
        // invariant more cleanly; this test is kept for traceability to the original spec.
    ...
    [Fact]
    public async Task ReleaseActiveAgentRunsAsync_CancelJobFailure_DoesNotPreventHandoff()
    {
    ...
    [Fact]
    public async Task ReleaseActiveAgentRunsAsync_ReleasesRunsFromTracking_NoLabelSwap()
    {
        // Label swaps are no longer performed during graceful shutdown — they caused agents to
        // abort on rolling updates. Verifies runs are released from tracking (ReleaseAgentRunsForHandoff)
        // and no label swap is attempted.
    ...
    // ── ReleaseActiveAgentRunsAsync Rolling-Update Safety ────────────────
    ...
    // Fix: ReleaseActiveAgentRunsAsync must NOT send CancelJob and must NOT write Cancelled
    // history. It should only release dedup guards so the new pod can dispatch / adopt the runs.

    [Fact]
    public async Task ReleaseActiveAgentRunsAsync_ReleasesAllRuns_EvenWhenAllAgentIdsAreNull()
    {
        // Regression: the old CancelActiveAgentRunsAsync had an early-return guard
        // filtering Where(r => r.AgentId != null). Sentinel runs (registered at dispatch
        // before JobAccepted sets AgentId) were invisible to it, so dedup guards were
        // never released if all active runs were sentinels.
        // Fix: ReleaseAgentRunsForHandoff is called unconditionally — no outer filter.
        var runService = new Orchestration.OrchestratorRunService(_mockLogger.Object);
        var sentinel1 = new PipelineRun
        {
            RunId = "sentinel-1",
            IssueIdentifier = "2001",
            IssueTitle = "Sentinel run 1",
            IssueProviderConfigId = "issue-1",
            RepoProviderConfigId = "repo-1",
            CurrentStep = PipelineStep.Created,
            HighWaterMark = PipelineStep.Created,
            StartedAt = DateTime.UtcNow,
            AgentId = null   // ← sentinel: AgentId not yet set
        };
        var sentinel2 = new PipelineRun
        {
            RunId = "sentinel-2",
            IssueIdentifier = "2002",
            IssueTitle = "Sentinel run 2",
            IssueProviderConfigId = "issue-1",
            RepoProviderConfigId = "repo-1",
            CurrentStep = PipelineStep.Created,
            HighWaterMark = PipelineStep.Created,
        ...
        runService.AddRun(sentinel1);
        runService.AddRun(sentinel2);
    ...
    [Fact]
    public async Task ReleaseActiveAgentRunsAsync_DoesNotSendCancelJobToAgents_WhenSenderIsWired()
    {
        // Arrange
        var mockRunService = new Mock<IOrchestratorRunService>();
        var agentRun = new PipelineRun
    ...
    [Fact]
    public async Task ReleaseActiveAgentRunsAsync_DoesNotWriteCancelledHistoryEntry()
    {
        // Arrange
        var mockRunService = new Mock<IOrchestratorRunService>();
        var agentRun = new PipelineRun
