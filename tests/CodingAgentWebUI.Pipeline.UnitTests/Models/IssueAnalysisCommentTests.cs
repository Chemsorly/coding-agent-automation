...
public class IssueAnalysisCommentTests
{
    ...
    [Fact]
    public void ToMarkdown_WithUnknownRecommendation_UsesRawValueAsLabel()
    {
        var comment = new IssueAnalysisComment
        {
            IssueTitle = "Test",
            ...
            EstimatedComplexity = string.Empty,
            ConfidenceAssessment = string.Empty,
            Assessment = new AnalysisAssessment { Recommendation = "defer" }
        };

        var md = comment.ToMarkdown();

        md.Should().Contain("📊 **Assessment: defer**");
        // Unknown recommendation uses the non-ready note (no "before approving")
        md.Should().Contain("*This analysis was generated by the agent after examining the codebase.*");
    }

    [Fact]
    public void ToMarkdown_WithNullRecommendation_UsesUnknownLabel()
    {
        var comment = new IssueAnalysisComment
        {
            IssueTitle = "Test",
            ...
            EstimatedComplexity = string.Empty,
            ConfidenceAssessment = string.Empty,
            Assessment = new AnalysisAssessment { Recommendation = "unknown_value_xyz" }
        };

        var md = comment.ToMarkdown();

        md.Should().Contain("📊 **Assessment: unknown_value_xyz**");
    }

    [Fact]
    public void ToMarkdown_NotReady_NullReason_DoesNotEmitReasonLine()
    {
        var comment = new IssueAnalysisComment
        {
            IssueTitle = "Test",
            ...
            EstimatedComplexity = string.Empty,
            ConfidenceAssessment = string.Empty,
            Assessment = new AnalysisAssessment { Recommendation = "not_ready", Reason = null }
        };

        var md = comment.ToMarkdown();

        md.Should().Contain("⚠️ **Assessment: Not Ready**");
        md.Should().NotContain("*Reason:");
    }
}
