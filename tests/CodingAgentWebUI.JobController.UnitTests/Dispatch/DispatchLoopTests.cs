...
public sealed class DispatchLoopTests
{
    ...
    public async Task WhenOneRunningAndOneCompletedJobForSameSelector_ConcurrencyCountIsOne()
    {
        ...
        var runningJob = new V1Job
        {
            ...
            Status = new V1JobStatus { Active = 1 }  // genuinely running
        };
    ...
    [Fact]
    public async Task WhenJobTerminalViaFailedCondition_IsNotCountedInConcurrency()
    {
        const string yaml = """
            - labels: dotnet10,opencode
              image: chemsorly/coding-agent:opencode-dotnet10
        ...
        // Failed job is terminal → concurrency = 0 < limit 1 → should dispatch
        _workItemClient.Verify(c => c.ClaimAsync(It.IsAny<Guid>(), It.IsAny<ClaimWorkItemRequest>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    /// </summary>
    // TODO: Add a test for `Conditions = null, Succeeded = 1` (distinct from `Conditions = []`).
    // Some K8s controllers omit the Conditions field entirely before updating pod counters; the
    // production code handles this (null check gates the loop, counter check runs regardless),
    // but there is no test verifying the counter fallback fires when Conditions is null rather
    // than an empty list. E.g.: Status = new V1JobStatus { Conditions = null, Succeeded = 1 }.
    [Fact]
    public async Task WhenJobTerminalViaSucceededCounter_IsNotCountedInConcurrency()
    {
        ...
        var succeededJobNoConditions = new V1Job
        {
            ...
            Status = new V1JobStatus
            {
                Conditions = [],  // explicitly empty — no Complete condition
                Succeeded = 1
            }
        };
    ...
    public async Task WhenJobTerminalViaFailedCounter_IsNotCountedInConcurrency()
    {
        ...
        var failedJobNoConditions = new V1Job
        {
            ...
            Status = new V1JobStatus
            {
                Conditions = [],  // explicitly empty — no Failed condition
                Failed = 1
            }
        };
