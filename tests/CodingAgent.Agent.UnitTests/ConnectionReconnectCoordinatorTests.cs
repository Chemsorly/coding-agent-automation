...
public sealed class ConnectionReconnectCoordinatorTests
{
    ...
    public async Task HandleTerminalClosedAsync_ConcurrentDispose_CASPreventsDoubleOwnership()
    {
        ...
        //   startEntered  — signalled by StartFunc as soon as StartAsync is entered, so the
        //                   main thread knows the reconnect loop is definitely inside StartAsync
        //                   before calling DisposeAsync.  Eliminates the timing heuristic.
        //   startBlocker  — held by StartFunc until the main thread releases it, keeping
        //                   StartAsync (and therefore the CAS) blocked until after DisposeAsync.
        ...
        var blockingHub = new FakeHubConnectionManager
        {
            StartFunc = _ =>
            {
                startEntered.TrySetResult(); // signal: we are now inside StartAsync
        ...
        // Wait until the reconnect loop is definitely inside StartAsync (guaranteed ordering)
