...
public class LocalPipelineExecutorPayloadTests
{
    ...
    {
        var run = MakeRun();
        run.AnalysisRecommendation = CodingAgent.Pipeline.Models.AnalysisGateResult.Ready;

        var payload = LocalPipelineExecutor.BuildCompletionPayload(run);

        payload.AnalysisRecommendation.Should().Be(CodingAgent.Pipeline.Models.AnalysisGateResult.Ready);
    }
