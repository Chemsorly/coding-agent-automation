...
public partial class AgentMonitoring : IAsyncDisposable
{
    ...
    // TODO: _priorityError is not cleared by non-priority actions (e.g. "Remove" button). A stale
    // error banner persists until the next successful priority update or page refresh. Consider
    // clearing _priorityError on any queue mutation, or adding a dismiss button to the error banner.
    ...
    private async Task HandleSetPriority((Guid Id, int Weight) args)
    {
        // TODO: No CancellationToken is forwarded here. If the component is disposed while the
        // priority HTTP request is in flight, the request and subsequent RefreshDataAsync will
        // still run against a potentially disposed component. Follow the _cts pattern used
        // elsewhere in this file if cancellation-on-disposal is needed.
        var (success, error) = await PageService.SetPriorityAsync(args.Id, args.Weight);
        if (!success)
        {
