...
public class AgentMonitoringPageService
{
    ...
    /// On success, refreshes the queue to reflect the authoritative server value.
    /// On failure (400, 409, network error), returns the error message so the caller
    /// can display it and the refresh also reverts any optimistic UI state.
    /// </summary>
    public async Task<(bool Success, string? Error)> SetPriorityAsync(Guid workItemId, int priorityWeight)
    {
        ...
        // returned, but a fast-fail ArgumentOutOfRangeException.ThrowIfGreaterThan / ThrowIfLessThan
        // check here would provide a clearer contract and avoid the unnecessary HTTP round-trip.
        try
        {
            // TODO: CancellationToken.None is used here rather than a caller-supplied token because
            // the Blazor event handler cannot easily supply one. If the component is disposed while
            // this request is in flight the HTTP call and RefreshDataAsync will still complete.
            // Accept a CancellationToken parameter and thread it through if cancellation-on-disposal
            // becomes a requirement.
            await _workItemClient.SetPriorityAsync(workItemId, priorityWeight, CancellationToken.None);
            await RefreshDataAsync();
            return (true, null);
        }
        catch (Exception ex)
        {
            Logger.Warning(ex, "Failed to set priority for WorkItem {WorkItemId}", workItemId);
            // TODO: ex.Message is forwarded to the UI. For HttpRequestException this is typically the
            // status code / reason phrase, but if a custom handler ever parses and echoes response
            // body content into the message, server-controlled text would appear verbatim in the UI.
            // Consider extracting only the status code or using a fixed error string instead.
            // TODO: If RefreshDataAsync throws here (e.g. network failure during refresh), its
            // exception will propagate out of SetPriorityAsync, replacing the original error and
            // breaking the (bool, string?) contract. Wrap the refresh in a nested try/catch if
            // robustness against double-failure is needed.
            await RefreshDataAsync();
