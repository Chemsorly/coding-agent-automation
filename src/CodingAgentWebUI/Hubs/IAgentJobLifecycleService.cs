...
public interface IAgentJobLifecycleService
{
    ...
    /// </summary>
    Task HandleJobAcceptedAsync(string jobId, AgentEntry? agent, CancellationToken ct);

    /// <summary>
    /// Handles job rejection: retry logic, re-queue or permanent failure, label swap, agent idle transition.
    /// </summary>
    Task HandleJobRejectedAsync(string jobId, AgentEntry? agent, string reason, CancellationToken ct);
    ...
    /// </summary>
    Task HandleJobCompletedAsync(string jobId, AgentEntry? agent, JobCompletionPayload payload, CancellationToken ct);
    ...
    /// </summary>
    void HandleStepTransition(string jobId, PipelineStep step, DateTimeOffset timestamp, Dictionary<string, string>? metadata);
}
