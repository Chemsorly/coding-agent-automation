...
using MergeResult = KiroWebUI.Pipeline.Models.MergeResult;
        ...
        IReadOnlyList<string>? blacklistedPaths, bool allowEmpty, CancellationToken ct)
    {
        ...
        return Task.Run(() =>
        {
            ...
            if (blacklistedPaths is { Count: > 0 })
            {
                ...
                {
                    if (KiroWebUI.Pipeline.Services.PipelineFormatting.IsPathBlacklisted(change.Path, blacklistedPaths))
                    {
