...
public static class JobSpecBuilder
{
    ...
    public sealed record BuildContext
    {
        ...
        /// <summary>
        /// Name of the per-Job K8s Secret that holds the pre-vended agent API key (Spec 043 Req 8a,
        /// fully implemented in issue #3034).
        ...
        /// from holding the master key and computing credentials for other pods.
        /// Null for non-work-item jobs (model-fetch, consolidation) which are not yet migrated.
        ///
        /// The stored value is <c>HMAC-SHA256(masterKey, jobName)</c>, generated by
        /// <c>DispatchLifecycleService</c> at dispatch time. The agent uses this value directly
        /// as its bearer token without further derivation — <c>HubConnectionManager</c> and
        /// <c>WorkItemHttpClient</c> no longer call <c>DeriveKey</c> for work-item pods.
        /// <c>AgentApiKeyAuthHandler</c> re-derives the expected value server-side from the
        /// <c>?agentId=</c> query param and compares — the master key never enters the pod.
        ///
        /// ⚠️ Do NOT set this for non-work-item pods (consolidation, model-fetch) — those pods
        /// receive the raw master key via <c>AGENT_API_KEY_FILE</c>, and <c>HubConnectionManagerFactory</c>
        /// derives <c>HMAC(master, agentId)</c> locally before constructing the connection. Injecting
        /// a pre-derived key here would cause double-derivation and authentication failure (infinite
        /// SignalR reconnect loop).
        /// TODO [WARNING]: The claim above about non-work-item derivation path depends on
        /// HubConnectionManagerFactory always being constructed with isWorkItemMode=false for those pods.
        /// If a future caller constructs HubConnectionManagerFactory with isWorkItemMode=true but
        /// DerivedKeySecretName=null, the pod will send the raw master key verbatim as the bearer token,
        /// which AgentApiKeyAuthHandler will reject. Ensure isWorkItemMode matches DerivedKeySecretName usage.
        /// See: Correctness [WARNING] — JobSpecBuilder doc comment invariant.
        /// </summary>
    ...
    private static List<V1EnvVar> BuildEnvVars(JobTemplate template, BuildContext ctx)
    {
        ...
        envVars.Add(new V1EnvVar
        {
            ...
            // The auth handler validates by computing HMAC(masterKey, AGENT_ID) server-side;
            // work-item pods receive the pre-derived value and present it verbatim.
