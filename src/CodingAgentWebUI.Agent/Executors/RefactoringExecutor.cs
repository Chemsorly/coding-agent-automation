...
public sealed class RefactoringExecutor
{
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            // 8. Return summary
            var summary = FormatRefactoringSummary(createdIssues);
        ...
        foreach (var proposal in proposalsToProcess)
        {
            try
            {
                ...
                var result = await issueProvider.CreateIssueAsync(
                    $"[Refactoring] {sanitizedTitle}",
                    body,
                    new[] { "refactoring", "agent-generated" },
                    ct);
    ...
    /// <summary>
    /// Formats the issue body from a refactoring proposal.
    /// Sanitizes user-controlled content to prevent markdown injection.
    /// </summary>
    private static string FormatIssueBody(RefactoringProposal proposal)
    {
        // WARNING 6 fix: Sanitize markdown in user-controlled fields
    ...
    /// </summary>
    internal static string FormatRefactoringSummary(IReadOnlyList<CreatedIssueInfo> createdIssues)
    {
