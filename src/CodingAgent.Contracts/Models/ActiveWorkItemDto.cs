...
public sealed record ActiveWorkItemDto
{
    ...
    /// in the Payload. Null when payload is absent, has no IssueDetail, or IssueDetail.Url is absent.
    /// Populated by the API for display purposes only — used by the Work page to link the issue
    /// number directly to the provider without requiring a separate API lookup.
    /// </summary>
