...
public sealed class JobQueueDrainService : BackgroundService
{
    ...
    private async Task<int> DrainPipelineJobsAsync(CancellationToken ct)
    {
        ...
        foreach (var agent in idleAgents)
        {
            ...
            if (await DispatchSingleJobAsync(agent, pendingJob, ct))
                dispatchedCount++;
        }
    ...
    private async Task<bool> DispatchSingleJobAsync(AgentEntry agent, PendingJob pendingJob, CancellationToken ct)
    {
        try
        {
            if (pendingJob.IsConsolidation)
            {
                return await HandleConsolidationJobAsync(agent, pendingJob, ct);
            }
            else
            {
                return await HandlePipelineJobAsync(agent, pendingJob, ct);
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task<bool> HandleConsolidationJobAsync(AgentEntry agent, PendingJob pendingJob, CancellationToken ct)
    {
    ...
    private async Task<bool> HandlePipelineJobAsync(AgentEntry agent, PendingJob pendingJob, CancellationToken ct)
    {
