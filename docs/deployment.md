...
| `workDistribution.dispatch.chatAgentJobTimeoutSeconds` | Max lifetime (seconds) of chat session K8s Jobs dispatched by `ChatJobDispatcher` (default: `7200`). Sets `activeDeadlineSeconds`. Agent work-item and consolidation jobs use `PipelineConfiguration.AgentTimeout` instead. |
