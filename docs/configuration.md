...
|---------------------------|---------|-------------|
| `workDistribution.dispatch.chatAgentJobTimeoutSeconds` | 7200 | Maximum lifetime (seconds) of chat session K8s Jobs dispatched by `ChatJobDispatcher`. Sets `activeDeadlineSeconds` on the Job spec — the pod is forcibly terminated by Kubernetes when this deadline passes. Minimum: 60s. Agent work-item and consolidation jobs use `PipelineConfiguration.AgentTimeout` (per-project overridable, default 30 min) as the single source of truth. |
