...
**Topology (verified 2026-09-25, updated for issue #3034):**
...
- Agent pods receive only: (a) a pre-vended per-job key (`HMAC-SHA256(master, jobName)`, generated server-side by `DispatchLifecycleService` and stored in a per-job K8s Secret — NOT mounted from the chart Secret), and (b) short-lived GitHub installation access tokens (1-hour expiry) vended via SignalR `RefreshToken` calls handled by `AgentTokenRefreshService` in the API hub. The raw PEM, the HMAC master key, and long-lived GitLab PATs never enter the agent container.
- Work-item pods receive `AGENT_API_KEY` from a per-job Secret (`caa-secrets-{workItemId[..8]}`) containing `HMAC-SHA256(masterKey, jobName)`. `HubConnectionManager` and `WorkItemHttpClient` present this value verbatim as the bearer token — no local derivation occurs. `AgentApiKeyAuthHandler` re-derives `HMAC(master, ?agentId=)` server-side and compares; the master key never leaves the server.
...
**`DerivedKeySecretName` usage (issue #3034):** `JobSpecBuilder.BuildContext.DerivedKeySecretName` MUST be set for work-item pods — `DispatchLifecycleService` sets it to the per-job Secret name before creating the Job. Do NOT set this for non-work-item pods (consolidation, model-fetch) — those agents still call `DeriveKey` internally and would double-derive if given a pre-derived key, causing authentication failure and an infinite SignalR reconnect loop. Those paths leave `DerivedKeySecretName = null`.
...
**Status (2026-09-25):** Implemented in issue #3034. `DispatchLifecycleService` now generates `HMAC-SHA256(master, jobName)` server-side at dispatch time and stores it in a per-job K8s Secret (`caa-secrets-{workItemId[..8]}`). `JobSpecBuilder` injects `AGENT_API_KEY` from that Secret (no master-key volume mount for work-item pods). `HubConnectionManager` and `WorkItemHttpClient` use the received `AGENT_API_KEY` directly as the bearer token — `DeriveKey` is no longer called on the agent side for work-item pods. The prior "Resolved in Spec 043" claim was false — that spec wrote the `DerivedKeySecretName` injection path in `JobSpecBuilder` but `DispatchLifecycleService` never set the property, and `JobSpecBuilder.Build()` threw when both `DerivedKeySecretName` and `WorkItemId` were non-null (added in #2234 to prevent double-derivation). Both blockers are now resolved.
